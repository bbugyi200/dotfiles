...
- Use `--format markdown` when the result is going into a prompt, transcript, or
  human-readable answer.
- Use `--format paths` only when you need source expression results or path-only output.
- Omit `--bob-dir` for Bryan's default vault. Include `--bob-dir <path>` only when the
  user asks for a nondefault vault.
- Treat command failures as actionable diagnostics. Fix the DQL, vault path, or command
  invocation; do not use failure as permission to scan and parse the whole vault ad hoc.
