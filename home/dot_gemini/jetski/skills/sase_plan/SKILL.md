...
```bash
sase skill use sase_plan --reason "<one-line reason for using this skill>"
```
...
2. **Write a self-contained plan** to `sase_plan_<name>.md` (descriptive underscore name).
   - You should construct the same type of implementation plan that you would have written in Gemini's native plan mode.
