...
3. **Write a commit message file** — Create the file at `.sase/commit_message.md`,
   ...
   into a whole-repository commit. **NEVER mention "Antigravity" or "Antigravity CLI"**
   — write as if a human authored the commit. Do not preemptively stash, fast-forward,
   pull, or hand-sync before committing; `sase stitch create` commits first, rebases
