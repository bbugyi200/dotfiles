...
Implicitly rendering a whole container, such as `{{ agents["build"].cfg }}`, produces compact JSON rather than a Python
representation. Jinja's `| tojson` filter remains available when explicit JSON formatting is preferred.
...
repeat chain) are not affected and can still read `{{ agents["name"].STOP }}`.
