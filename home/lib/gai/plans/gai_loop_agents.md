...
Add background execution of `crs` and `fix-hook` workflows to `gai loop` that run every 10s with hook checks. Workflows
run in parallel, auto-accept proposals on completion, and each gets its own workspace.
