...
def build_completeness_checker_prompt(state: CreateProjectState) -> str:
    """Build the prompt for the completeness checker agent."""
    ...
    prompt = f"""You are an expert project completeness reviewer. Your goal is to carefully assess a generated project plan and ensure it is complete, correct, and ready for implementation.
...
@{projects_file} - The project plan generated by the planner agent

# YOUR TASK:

**THINK EXTREMELY HARD** about this task. This ProjectSpec will be used to perform actual work, so completeness and correctness are critical.
...
2. **Review the prior work** (if available) to understand:
   ...
   - **CRITICAL**: You must NOT propose CLs for work that has already been completed
   - What patterns and approaches have been used
   - What remains to be done

3. **Carefully assess the current project plan**:
   - Does it cover ALL requirements from the design documents?
   - Are there any missing CLs for features/requirements not yet addressed?
   - Are the CLs appropriately sized (small and focused)?
   - Should any CLs be split into smaller, more focused CLs?
   - Should any CLs be merged because they're too granular?
   - Do the CLs have correct dependencies (PARENT fields)?
   - Do the CLs have appropriate test coverage?
   - Are there any CLs that duplicate work already completed (from prior work)?

4. **Generate a complete and correct project plan**:
   - Include ALL ChangeSpecs from the current plan that are still valid
   - Add new ChangeSpecs for any missing requirements
   - Modify existing ChangeSpecs if they need to be split, merged, or corrected
   - Remove or modify any ChangeSpecs that duplicate already-completed work
   - Ensure every design requirement is satisfied
   - Ensure CLs are small and focused
   - Ensure almost all CLs include test coverage
   - Verify that PARENT dependencies are correct

# CRITICAL REQUIREMENTS:

- **THINK EXTREMELY HARD** - This plan will be used for actual implementation
- **Be VERY CAREFUL** not to add CLs for work already completed (check prior work analysis)
...
- **You CAN and SHOULD modify the plan** if you find issues:
  - Add missing CLs
  - Split overly large CLs
  - Merge overly granular CLs
  - Fix incorrect descriptions or dependencies
  - Remove CLs for already-completed work

# OUTPUT FORMAT:

Your response MUST be a COMPLETE ProjectSpec (including the BUG field) in the following format:
...
PARENT: <PARENT2>
CL: <CL2>
TEST TARGETS: <TEST_TARGETS2>
STATUS: <STATUS2>
```

**CRITICAL**:
- Start with the BUG field from the current project plan
- Separate each ChangeSpec with a blank line
- Include ALL ChangeSpecs (both existing ones you're keeping and new ones you're adding)
- Your output will REPLACE the current project plan entirely
...
5. **TEST TARGETS**: This field is OPTIONAL. You can:
   ...
   - **Specify one or more bazel/test targets** (space-separated) if you know exactly which tests need to pass
...
- Do NOT include CLs for any work already completed (found in prior work)
...
- **Your output must be a COMPLETE ProjectSpec** that will replace the current plan

Begin your completeness review and generate the final, complete project plan now:
"""
