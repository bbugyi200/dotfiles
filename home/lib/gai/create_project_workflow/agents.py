...
from .prompts import build_completeness_checker_prompt, build_planner_prompt
...
def run_completeness_checker_agent(state: CreateProjectState) -> CreateProjectState:
    """Run the completeness checker agent to review and complete the project plan."""
    print_status("Running completeness checker agent...", "progress")

    # Build prompt for completeness checker
    prompt = build_completeness_checker_prompt(state)

    # Send prompt to Gemini with big model (same as planner)
    model = GeminiCommandWrapper(model_size="big")
    model.set_logging_context(
        agent_type="completeness_checker",
        iteration=1,
        workflow_tag=state.get("workflow_tag"),
        artifacts_dir=state.get("artifacts_dir"),
    )
    ...
    print_status("Completeness checker agent response received", "success")

    # Save the completeness checker agent's response
    artifacts_dir = state["artifacts_dir"]
    checker_response_path = os.path.join(
        artifacts_dir, "completeness_checker_response.txt"
    )
    with open(checker_response_path, "w") as f:
        f.write(ensure_str_content(response.content))

    print(f"Saved completeness checker response to: {checker_response_path}")
    ...
    if not is_valid:
        print_status(
            f"Completeness checker plan validation failed: {error_message}", "error"
        )
        return {
            **state,
            "success": False,
            "failure_reason": f"Completeness checker plan validation failed: {error_message}",
            "messages": state["messages"] + messages + [response],
        }

    # Write the updated project plan to the projects file (replacing the previous plan)
    projects_file = state["projects_file"]
    bug_id = state["bug_id"]
    try:
        with open(projects_file, "w") as f:
            ...
            # Write the rest of the project plan
            f.write(ensure_str_content(response.content))
        print_status(f"Updated project plan written to: {projects_file}", "success")
    except Exception as e:
        print_status(f"Failed to write updated project plan: {e}", "error")
        return {
            **state,
            "success": False,
            "failure_reason": f"Failed to write updated project plan to {projects_file}: {str(e)}",
            "messages": state["messages"] + messages + [response],
        }
