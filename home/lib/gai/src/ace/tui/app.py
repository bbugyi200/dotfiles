...
class AceApp(BaseActionsMixin, HintActionsMixin, App[None]):
    ...
    def _start_agent_for_cl(
        ...
        if project_name is None:
            self.notify(f"Could not determine project for {cl_name}", severity="error")
            return
    ...
    def _start_agent_common(
        ...
        if not os.path.isfile(project_file):
            self.notify(f"Project file not found: {project_file}", severity="error")
            return
        ...
        if not claim_workspace(
            ...
            self.notify("Failed to claim workspace", severity="error")
            return
        ...
        try:
            ...
            # Clean workspace
            self.notify(f"Cleaning workspace {workspace_num}...")
            run_bb_hg_clean(workspace_dir, f"{display_name}-ace")

            # Update workspace
            self.notify(f"Updating to {update_target}...")
            ...
            if result.returncode != 0:
                ...
                self.notify(f"bb_hg_update failed: {error_msg}", severity="error")
            ...
            if prompt is None:
                self.notify("No prompt provided - cancelled", severity="warning")
                release_workspace(project_file, workspace_num, workflow_name, cl_name)
                return
            ...
            # Refresh agents list
            self._load_agents()
            self.notify(f"Agent started for {display_name}")

        except subprocess.TimeoutExpired:
            self.notify("bb_hg_update timed out", severity="error")
            release_workspace(project_file, workspace_num, workflow_name, cl_name)
        except Exception as e:
