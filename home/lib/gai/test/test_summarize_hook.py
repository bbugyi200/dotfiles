...
from work.changespec import HookEntry, HookStatusLine
from work.hooks import get_failing_hooks_for_fix
from work.hooks.operations import get_failing_hooks_for_summarize
...
def test_get_failing_hooks_for_summarize_proposal_entry() -> None:
    ...
    hook = _make_hook_with_status(
        command="make test",
        history_entry_num="2a",  # Proposal entry
        status="FAILED",
        suffix=None,
    )
...
def test_get_failing_hooks_for_summarize_proposal_entry_letter_b() -> None:
    ...
    hook = _make_hook_with_status(
        command="make lint",
        history_entry_num="3b",  # Proposal entry with 'b'
        status="FAILED",
        suffix=None,
    )
    result = get_failing_hooks_for_summarize([hook])
...
def test_get_failing_hooks_for_summarize_regular_entry() -> None:
    ...
    hook = _make_hook_with_status(
        command="make test",
        history_entry_num="2",  # Regular entry (no letter)
        status="FAILED",
        suffix=None,
    )
    result = get_failing_hooks_for_summarize([hook])
...
def test_get_failing_hooks_for_fix_regular_entry() -> None:
    ...
    hook = _make_hook_with_status(
        command="make test",
        history_entry_num="2",  # Regular entry
        status="FAILED",
        suffix=None,
    )
    result = get_failing_hooks_for_fix([hook])
