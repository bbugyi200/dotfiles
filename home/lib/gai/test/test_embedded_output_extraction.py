...
def _make_ewf(ctx: dict[str, object]) -> EmbeddedWorkflowResult:
    """Create an EmbeddedWorkflowResult with the given context for testing."""
    return EmbeddedWorkflowResult(
        workflow_name="test",
        pre_steps=[],
        post_steps=[],
        context=ctx,
...
def test_normal_propose_context() -> None:
    ...
    diff_path, meta = _extract_embedded_outputs([_make_ewf(ctx)])
...
def test_multiple_embedded_workflows_last_diff_path_wins() -> None:
    ...
    diff_path, meta = _extract_embedded_outputs([_make_ewf(ctx1), _make_ewf(ctx2)])
