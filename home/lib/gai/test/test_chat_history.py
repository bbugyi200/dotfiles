...
def test_list_chat_histories_with_branch_filter() -> None:
    """Test list_chat_histories filters by branch/workspace prefix."""
    with tempfile.TemporaryDirectory() as tmpdir:
        test_chats_dir = os.path.join(tmpdir, "chats")
        os.makedirs(test_chats_dir)

        # Create test files for different branches
        files = [
            "master-run-251128120000.md",
            "master-rerun-251128130000.md",
            "feature-branch-run-251128140000.md",
            "other-branch-run-251128150000.md",
        ]
        for filename in files:
            filepath = os.path.join(test_chats_dir, filename)
            with open(filepath, "w") as f:
        ...
        with patch("chat_history._get_chats_directory", return_value=test_chats_dir):
            # Filter by master branch
            result = list_chat_histories(branch_or_workspace="master")
            assert len(result) == 2
            assert all(name.startswith("master-") for name in result)

            # Filter by feature-branch
            result = list_chat_histories(branch_or_workspace="feature-branch")
            assert len(result) == 1
            assert result[0] == "feature-branch-run-251128140000"
...
def test_list_chat_histories_filter_no_matches() -> None:
    """Test list_chat_histories returns empty when filter has no matches."""
    with tempfile.TemporaryDirectory() as tmpdir:
        test_chats_dir = os.path.join(tmpdir, "chats")
        os.makedirs(test_chats_dir)

        # Create test files
        files = ["master-run-251128120000.md"]
        for filename in files:
            filepath = os.path.join(test_chats_dir, filename)
            with open(filepath, "w") as f:
                f.write("content")
        ...
        with patch("chat_history._get_chats_directory", return_value=test_chats_dir):
            result = list_chat_histories(branch_or_workspace="nonexistent-branch")
            assert result == []
