...
from search.loop.checks_runner import (
...
def test_get_pending_checks_no_directory() -> None:
    ...
    with patch(
        "search.loop.checks_runner._get_checks_directory",
...
def test_get_pending_checks_with_matching_files() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            "search.loop.checks_runner._get_checks_directory", return_value=temp_dir
        ):
            result = _get_pending_checks(mock_changespec)
...
def test_check_pending_checks_processes_completed() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch(
            ...
            with patch("search.loop.checks_runner.update_last_checked"):
                with patch(
                    "search.loop.checks_runner.is_parent_submitted"
                ) as mock_parent:
