...
def test_mentor_config_dataclass() -> None:
    ...
    config = MentorConfig(mentor_name="test", xprompt="test prompt")

    assert config.mentor_name == "test"
    assert config.xprompt == "test prompt"
...
def test_mentor_config_run_on_wip_true() -> None:
    ...
    config = MentorConfig(mentor_name="test", xprompt="test prompt", run_on_wip=True)
...
def test_mentor_profile_config_with_diff_regexes() -> None:
    ...
    mentors = [MentorConfig(mentor_name="mentor1", xprompt="Prompt 1")]
    profile = MentorProfileConfig(
        profile_name="test_profile",
        mentors=mentors,
