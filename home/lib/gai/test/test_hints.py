...
# --- Tests for parse_copy_input ---


def test_parse_copy_input_empty() -> None:
    """Empty input returns empty results."""
    targets, use_multi, invalid = parse_copy_input("")
    assert targets == []
    assert use_multi is False
    assert invalid == []


def test_parse_copy_input_single_target() -> None:
    """Single valid target returns it without multi-format."""
    targets, use_multi, invalid = parse_copy_input("2")
    assert targets == [2]
    assert use_multi is False
    assert invalid == []


def test_parse_copy_input_multiple_targets() -> None:
    """Multiple targets enables multi-format."""
    targets, use_multi, invalid = parse_copy_input("1 3 5")
    assert targets == [1, 3, 5]
    assert use_multi is True
    assert invalid == []


def test_parse_copy_input_with_at_suffix() -> None:
    """@ suffix forces multi-format for single target."""
    targets, use_multi, invalid = parse_copy_input("2@")
    assert targets == [2]
    assert use_multi is True
    assert invalid == []
...
def test_parse_copy_input_range() -> None:
    """Range expands to all numbers and enables multi-format."""
    targets, use_multi, invalid = parse_copy_input("1-3")
    assert targets == [1, 2, 3]
    assert use_multi is True
    assert invalid == []


def test_parse_copy_input_range_with_at() -> None:
    """Range with @ suffix."""
    targets, use_multi, invalid = parse_copy_input("2-4@")
    assert targets == [2, 3, 4]
    assert use_multi is True
    assert invalid == []


def test_parse_copy_input_invalid_targets() -> None:
    """Out-of-range targets are reported as invalid."""
    targets, use_multi, invalid = parse_copy_input("1 6 10")
    assert targets == [1]
    assert use_multi is False  # Only one valid target
    assert invalid == [6, 10]


def test_parse_copy_input_invalid_targets_with_valid() -> None:
    """Mix of valid and invalid targets."""
    targets, use_multi, invalid = parse_copy_input("1 3 7")
    assert targets == [1, 3]
    assert use_multi is True  # Two valid targets
    assert invalid == [7]


def test_parse_copy_input_zero_target() -> None:
    """Zero is invalid (targets are 1-indexed)."""
    targets, use_multi, invalid = parse_copy_input("0")
    assert targets == []
    assert use_multi is False
    assert invalid == [0]


def test_parse_copy_input_no_duplicates() -> None:
    """Duplicate targets are ignored."""
    targets, use_multi, invalid = parse_copy_input("1 1 2 2")
    assert targets == [1, 2]
    assert use_multi is True
    assert invalid == []


def test_parse_copy_input_mixed_range_and_single() -> None:
    """Mix of range and single numbers."""
    targets, use_multi, invalid = parse_copy_input("1-2 4")
    assert targets == [1, 2, 4]
    assert use_multi is True
    assert invalid == []


def test_parse_copy_input_custom_max_target() -> None:
    """Custom max_target parameter works."""
    targets, use_multi, invalid = parse_copy_input("3", max_target=2)
    assert targets == []
    assert use_multi is False
    assert invalid == [3]

    targets, use_multi, invalid = parse_copy_input("3", max_target=5)
    assert targets == [3]
    assert use_multi is False
    assert invalid == []


def test_parse_copy_input_standalone_at() -> None:
    """Standalone @ is skipped but sets multi-format flag."""
    targets, use_multi, invalid = parse_copy_input("@")
    assert targets == []
    assert use_multi is True  # @ was detected
    assert invalid == []
