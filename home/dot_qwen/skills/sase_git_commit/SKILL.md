...
3. **Write a commit message file** — Create a file (e.g., `commit_message.md`) containing the commit message. **NEVER
   mention "Qwen" or "Qwen Code"** — write as if a human authored the commit. Do not preemptively stash, fast-forward,
   pull, or hand-sync before committing; `sase commit` commits first, rebases automatically, and handles mechanical
