...
3. **Write a commit message file** — Create the file at `.sase/commit_message.md`, relative to the repository being
   ...
   commit finalizer and can never be swept into a whole-repository commit. **NEVER mention "OpenCode"** — write as if a
   human authored the commit. Do not preemptively stash, fast-forward, pull, or hand-sync before committing;
   `sase commit` commits first, rebases automatically, and handles mechanical bead-store conflicts.
