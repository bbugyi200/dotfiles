...
complete. Also, run `tools/sase_sbd show` on every child bead an ensure that any notes on those beads have been
addressed.

If not, plan out the remaining work and complete it. Otherwise, close the bead using the `tools/sase_sbd close` command.
