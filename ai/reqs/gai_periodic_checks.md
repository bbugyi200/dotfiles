...
The `gai work` command should periodically check mailed ChangeSpecs for status updates (submission, comments) and
automatically update their status when appropriate.
...
- [x] Run `is_cl_submitted` and `critique_comment` from the correct workspace directory
      (`$GOOG_CLOUD_DIR/<project>/$GOOG_SRC_DIR_BASE`)
